...
CREATE TABLE IF NOT EXISTS validation.scenarios (
    ...
    mutation_type          STRING  COMMENT 'noop | delete_rows | insert_extra_rows | update_column | add_extra_column | drop_column | null_out_pk | clear_key_cols | delete_rows_no_pk | insert_extra_rows_no_pk | content_swap_no_pk',
    mutation_params        STRING  COMMENT 'JSON params for the mutation (e.g. {"count": 50, "column": "notes"})',
    expected_status        STRING  COMMENT 'pass | inconclusive | count_mismatch | hash_mismatch | schema_drift | error',
    valid_comparison_modes STRING  COMMENT 'Comma-separated list: basic,hash,hash_no_pk,advanced,advanced_no_pk. Scenarios skipped (not_applicable) if active mode not listed',
...
SCENARIOS = [
    ...
    # ── No-PK validation (Phases 3, 4, 5 of feature/no-pk) ──────────────────
    # event_log: PK-less fixture wired to run_hash_no_pk via
    # pk_fallback_strategy='no_pk_hash'. Mutations use the no_pk variants
    # which operate via row-equality predicates instead of PK lookups.
    Row(
        scenario_id="pk-less-pass-event-log",
        description="No mutation on PK-less event_log → engine reports pass via run_hash_no_pk",
        target_table="event_log",
        mutation_type="noop",
        mutation_params="{}",
        expected_status="pass",
        valid_comparison_modes="hash_no_pk",
        enabled=True,
        created_at=NOW,
    ),
    Row(
        scenario_id="pk-less-extra-row-event-log",
        description="Append 1 unique row to LH event_log → count_mismatch with only_in_lakehouse_multiset sample",
        target_table="event_log",
        mutation_type="insert_extra_rows_no_pk",
        mutation_params='{"count": 1, "mutate": true}',
        expected_status="count_mismatch",
        valid_comparison_modes="hash_no_pk",
        enabled=True,
        created_at=NOW,
    ...
    Row(
        scenario_id="pk-less-missing-row-event-log",
        description="Delete 1 row from LH event_log → count_mismatch with only_in_postgres_multiset sample",
        target_table="event_log",
        mutation_type="delete_rows_no_pk",
        mutation_params='{"count": 1}',
        expected_status="count_mismatch",
        valid_comparison_modes="hash_no_pk",
        enabled=True,
        created_at=NOW,
    ...
    Row(
        scenario_id="pk-less-dup-count-diff-event-log",
        description="Append 1 verbatim duplicate to LH event_log → count_mismatch with multiset_count_diff sample",
        target_table="event_log",
        mutation_type="insert_extra_rows_no_pk",
        mutation_params='{"count": 1, "mutate": false}',
        expected_status="count_mismatch",
        valid_comparison_modes="hash_no_pk",
        enabled=True,
        created_at=NOW,
    ...
    Row(
        scenario_id="pk-less-content-swap-event-log",
        description="Delete 1 + insert 1 mutated on LH event_log → totals match, multiset diverges → INCONCLUSIVE",
        target_table="event_log",
        mutation_type="content_swap_no_pk",
        mutation_params='{"count": 1}',
    ...
    # landing_orders: PK-less fixture wired to run_advanced_no_pk via
    # pk_fallback_strategy='no_pk_advanced'. exceptAll-based diff surfaces
    # both directions; content_swap appears as hash_mismatch (count_match=True).
    Row(
        scenario_id="pk-less-pass-landing-orders",
        description="No mutation on PK-less landing_orders → engine reports pass via run_advanced_no_pk",
        target_table="landing_orders",
        mutation_type="noop",
        mutation_params="{}",
        expected_status="pass",
        valid_comparison_modes="advanced_no_pk",
        enabled=True,
        created_at=NOW,
    ),
    Row(
        scenario_id="pk-less-content-swap-landing-orders",
        description="Delete 1 + insert 1 mutated on LH landing_orders → totals match → hash_mismatch (advanced_no_pk surfaces both)",
        target_table="landing_orders",
        mutation_type="content_swap_no_pk",
        mutation_params='{"count": 1}',
        expected_status="hash_mismatch",
